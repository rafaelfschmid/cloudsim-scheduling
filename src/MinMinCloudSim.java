...
public class MinMinCloudSim {
	...
	private static void segmentedSorting(float machines[], int task_index[],
		...
		new MinMinCloudSim().segmented_sort(machines, task_index, segments, t,	m);
	}
	...
	private static void min_min_cpu(DatacenterBroker broker) {
		...
		Log.printLine("Completion times:");
		for (int k = 0; k < completion_times.length; k++) {
			Log.printLine(completion_times[k]);
		}
	}
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			...
			try {
				...
				@SuppressWarnings("unused")
				Datacenter datacenter3 = createDatacenter("Datacenter_2");				
				@SuppressWarnings("unused")
				Datacenter datacenter4 = createDatacenter("Datacenter_2");
				@SuppressWarnings("unused")
				Datacenter datacenter5 = createDatacenter("Datacenter_2");
				...
				// VM description
				// int vmid = 0;
				// int number_of_vms = Integer.parseInt(br.readLine());
				// int total_mips = Integer.parseInt(br.readLine());
				int number_of_vms = Integer.parseInt(br_machines.readLine());
				//int total_mips = Integer.parseInt(args[2]);
				//float pa = (1 + number_of_vms) * number_of_vms / 2;
				//float mips = total_mips / pa;
				...
				for (int vmid = 0; vmid < number_of_vms; vmid++) {
					// create VMs
					float mips = Float.parseFloat(br_machines.readLine()); 
				...
				if (args[0].compareTo("gpu") == 0) {
					startTime = System.currentTimeMillis();
					min_min_gpu(broker);
					stopTime = System.currentTimeMillis();
				} else if (args[0].compareTo("cpu") == 0) {
					startTime = System.currentTimeMillis();
					min_min_cpu(broker);
					stopTime = System.currentTimeMillis();
				}
